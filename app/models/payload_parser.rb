module PayloadParser
	...
	def payload_valid?(params)
		pr = payload_request_generator("new", params)
		pr.valid?
	end

	def payload_exists?(params)
		payload_request_generator("exists?", params)
	end

	def add_to_database(params, identifier)
		params = params_parser(params, identifier)
		payload_request_generator("create", params)
	end

	def payload_request_generator(method, params)
		platform = UserAgent.parse(params['u_agent']).platform
		browser = UserAgent.parse(params['u_agent']).browser
		PayloadRequest.send(method, url: Url.find_or_create_by(address: params['url']),
